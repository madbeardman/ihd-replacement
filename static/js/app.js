...
function renderAgileChart(data) {
    ...
    chart.classList.toggle("agile-chart-mixed", hasNegative);
    chart.classList.toggle("agile-chart-positive-only", !hasNegative);
    ...
    if (hasNegative) {
        ...
        for (const slot of data.slots) {
            ...
            bar.className = `agile-bar positive ${slot.band}${slot.is_now ? " now" : ""}`;
            bar.style.height = `${finalHeight}%`;
            bar.dataset.price = formatPrice(slot.value_inc_vat);
            bar.title =
                `${slot.source_day} slot ${slot.source_index}\n` +
    ...
    data.slots.forEach((slot, index) => {
        ...
        if (minutes === 0 && wraps[index]) {
            ...
            const barCenter = wrapRect.left - chartRect.left + wrapRect.width / 2;
